...
/**
 * Stage G Routes - Handle Stage G database operations
 * Amount: 50
 */
...
// GET /api/v1/stage-g/last-game-id - Get last game ID from stage_g table
router.get('/last-game-id', async (req, res) => {
  try {
    console.log('🔍 DB Manager: Received request for last game ID from Stage G');
    ...
    const stageGModel = modelManager.getModel('StageG');
    if (!stageGModel) {
      throw new Error('StageG model not found');
    }
    
    // Update existing records with new business logic
    await updateExistingRecords(modelManager, 'G');
    
    console.log('📋 Querying stage_g table for highest game ID...');
    
    const query = `
      SELECT * FROM ${stageGModel.tableName} 
      ORDER BY id DESC, createdAt DESC 
      LIMIT 1
    `;
    
    const lastRecord = modelManager.executeQuery('StageG', query);
    
    if (lastRecord && lastRecord.length > 0) {
      const gameData = lastRecord[0];
      console.log('✅ Found existing game record:', gameData);
      
      const businessValues = calculateBusinessValues('G', gameData.playerId, gameData.selectedBoard);
      ...
      res.json({
        ...
        source: 'database',
        table: stageGModel.tableName,
        stage: 'stage-g'
      });
      
    } else {
      // Create first game ID with Stage G business logic (amount = 50)
      console.log('📝 No records found, creating first game ID for Stage G...');
      
      const firstGameId = `GAME_${Date.now()}_G001`;
      ...
      const businessValues = calculateBusinessValues('G', samplePlayerIds, sampleSelectedBoard);
      ...
      const firstRecord = {
        ...
        metadata: JSON.stringify({
          ...
          businessRules: 'stage_g_applied'
        })
      };
      
      const insertQuery = `
        INSERT INTO ${stageGModel.tableName} (
      ...
      const result = modelManager.executeRun('StageG', insertQuery, insertParams);
      
      if (result && result.lastInsertRowid) {
        const newRecord = modelManager.executeQuery('StageG', 
          `SELECT * FROM ${stageGModel.tableName} WHERE id = ?`, 
          [result.lastInsertRowid]
        );
        
        console.log('✅ Created first Stage G game record with business logic:', newRecord[0]);
        ...
        res.json({
          ...
          data: {
            ...
            isNew: true,
            message: 'First game ID created with Stage G business logic'
          },
          source: 'database_new',
          table: stageGModel.tableName,
          stage: 'stage-g'
    ...
    console.error('❌ Error getting last game ID from Stage G:', error.message);
    res.status(500).json({
      success: false,
      error: 'Failed to get last game ID from Stage G',
...
// POST /api/v1/stage-g/create - Create new Stage G record
router.post('/create', async (req, res) => {
  try {
    ...
    if (result && result.lastInsertRowid) {
      ...
      console.log('✅ Created new Stage G record with business logic:', newRecord[0]);
      ...
      res.json({
        ...
        message: 'Stage G record created with business logic',
        stage: 'stage-g'
      });
    } else {
      throw new Error('Failed to create Stage G record');
    }
    
  } catch (error) {
    console.error('❌ Error creating Stage G record:', error.message);
    res.status(500).json({
      success: false,
      error: 'Failed to create Stage G record',
      details: error.message
    });
  }
});

// GET /api/v1/stage-g/status - Check Stage G table status
router.get('/status', async (req, res) => {
  try {
    ...
    const statsQuery = `
      ...
      FROM ${stageGModel.tableName}
    `;
    
    const stats = modelManager.executeQuery('StageG', statsQuery);
    ...
    res.json({
      ...
      data: {
        ...
        model: 'StageG',
        amount: STAGE_AMOUNTS.G
      },
      stage: 'stage-g'
    });
    
  } catch (error) {
    console.error('❌ Error getting Stage G status:', error.message);
    res.status(500).json({
      success: false,
      error: 'Failed to get Stage G status',
      details: error.message
    });
  }
});
